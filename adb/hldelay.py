...
if __name__ == "__main__":
  ...
  for opt, arg in opts:
    if opt in ["-d","--delay"]:
      ...
      print("Ignoring CFG_DELAY_RANGE (use -c for exhaustive search)")
...
def testAdb(entryVar=None):
  ...
  try:
    if entryVar is not None:
      proc = subprocess.run(["adb","shell","/data/local/tmp/supersoaker","%d" % (entryVar)],capture_output=True,timeout=10);
    else:
      proc = subprocess.run(["adb","shell","/data/local/tmp/supersoaker"],capture_output=True,timeout=10);
    time.sleep(2.0)
    stdout = proc.stdout
    stderr = proc.stderr
    print(stdout)
...
while glitchCtr <= CFG_COUNT:
  ...
  delay_time = CFG_DELAY + glitchCtr # random.randint(CFG_DELAY,CFG_DELAY + CFG_DELAY_RANGE)
  if doResetAll:
