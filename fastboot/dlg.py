...
# fastboot download glitch (fastboot boot loc_0.bin)
...
def testProcess():
  proc = subprocess.run(["fastboot","boot","boot.img"],capture_output=True,timeout=4)
  try:
    time.sleep(3.0)
    stdout = proc.stdout
    stderr = proc.stderr
...
# flashing unlock
capturemask = [ord(x) for x in "download:00001800"]
# usb descriptor
